...
const range = function(count, skipZero, descending) {
  ...
  // count is length of resulting array
  // array.from give it the length of the array and start index give us results
  // check descending, we reverse the array
  // each time we loop over we push the value
  let arr = [];
  let startingNum = skipZero ? 1 : 0;
  let length =  count;

  if(typeof(count) !== "number") return [];

  arr = Array.from({ length }, (_, i) => {
    startingNum + i
  });  
